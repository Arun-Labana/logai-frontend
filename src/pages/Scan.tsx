    ...
    <div className="animate-fade-in max-w-2xl mx-auto">
      ...
      {scanComplete && scanResult && (
        <div className={`rounded-xl p-6 ${
          scanResult.logs_scanned === 0 
            ? 'bg-logai-warning/10 border border-logai-warning/30' 
            : 'bg-logai-success/10 border border-logai-success/30'
        }`}>
          <div className="flex items-center gap-3 mb-4">
            {scanResult.logs_scanned === 0 ? (
              <>
                <XCircle className="w-6 h-6 text-logai-warning" />
                <h3 className="text-lg font-semibold text-logai-text-primary">
                  No Logs Found
                </h3>
              </>
          ...
          {scanResult.logs_scanned === 0 ? (
            <div className="space-y-4">
              <p className="text-logai-text-secondary">
                No error logs found in the selected time range. Make sure your Java application is:
              </p>
              <ol className="text-sm text-logai-text-secondary list-decimal list-inside space-y-2">
                <li>Configured with this app's ID in <code className="text-logai-accent">logback.xml</code></li>
                <li>Running and generating logs</li>
                <li>Sending logs to Supabase (check console for errors)</li>
              </ol>
              <div className="bg-logai-bg-secondary rounded-lg p-3">
                <p className="text-xs text-logai-text-secondary mb-1">App ID:</p>
                <code className="text-xs text-logai-accent break-all">{appId}</code>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex gap-3">
                ...
                {scanResult.cluster_ids && scanResult.cluster_ids.length > 0 && (
                  <Link
